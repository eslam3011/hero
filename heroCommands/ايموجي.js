export default {
  ...
  onStart: async function ({
    ...
    const startNewGame = async () => {
      ...
      await message.reply("🎯 لعبة جديدة! أول من يرسل هذا الإيموجي يفوز!\n\n📝 السؤال: " + new_random_question.question);
    ...
    const setupListener = () => {
      stopListening = api.listenMqtt((err, event) => {
        ...
        if (event.type === "message") {
          ...
          if (event.body === currentQuestion.answer) {
            message.reply("🎉 مبروك! قام " + event.senderName + " بكتابة الإجابة الصحيحة: " + currentQuestion.answer);
          ...
          // إذا كانت الرسالة تحتوي على إيموجي لكنها خاطئة
            ...
            console.log("❌ الإجابة غلط - أرسل " + event.senderName + ": " + event.body + " والإجابة الصحيحة هي: " + currentQuestion.answer);
          }
        }
      });
    };
